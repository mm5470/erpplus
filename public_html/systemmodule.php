...
if($act=="del")
	{
		$delid=$_GET["id"];		
	    $sqld="delete from mk_systemmodule where id ='".$delid."'";
	    $db->query($sqld);
...
if (isset($_GET['page'])) {
	$page = $_GET['page'];
	  ...
	  $sqlstr="select * from mk_systemmodule  order by  id desc";
	 // echo $sqlstr;
	  if (isset($_GET['total'])) {
	  	$total = $_GET['total'];
		}
