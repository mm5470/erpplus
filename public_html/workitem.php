...
if($act=="del")
	{
		$delid=$_GET["id"];		
	    $sqld="delete from mk_workitem where id ='".$delid."'";
	    $db->query($sqld);
...
if (isset($_GET['page'])) {
	$page = $_GET['page'];
	  ...
	  $sqlstr="select mk_workitem.*,mk_systemmodule.name as systemname from mk_workitem left join mk_systemmodule on mk_systemmodule.id=mk_workitem.systemid order by mk_workitem.id desc";
	 // echo $sqlstr;
	  if (isset($_GET['total'])) {
	  	$total = $_GET['total'];
		}
