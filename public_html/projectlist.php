...
if($act=="del")
	{
		$delid=$_GET["id"];		
	    $sqld="delete from mk_project where id ='".$delid."'";
	    $db->query($sqld);
...
if (isset($_GET['page'])) {
	$page = $_GET['page'];
	  ...
	  $sqlstr="select mk_project.*,mk_projectstatus.status from mk_project left join mk_projectstatus on mk_project.projectnum=mk_projectstatus.projectnum order by mk_project.id desc,mk_project.adddate desc";
	 // echo $sqlstr;
	  if (isset($_GET['total'])) {
	  	$total = $_GET['total'];
		}
