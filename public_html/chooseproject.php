...
if (isset($_GET['page'])) {
	$page = $_GET['page'];
	  ...
	  $sqlstr="select * from mk_project where 1=1 ";
	   if($searchtitle<>""){$sqlstr=$sqlstr." and mk_project.name like '%".$searchtitle."%'";}
	  if($searchnum<>""){$sqlstr=$sqlstr." and mk_project.projectnum='".$searchnum."'";}
	  $sqlstr=$sqlstr." order by  adddate desc,projectnum desc";
	  if (isset($_GET['total'])) {
	  	$total = $_GET['total'];
		}
	 else {
	...
	$phpfile="chooseproject.php?searchtitle=$searchtitle&searchnum=$searchnum&getid=$getid&getname=$getname&page=";	
	$pagearray=pagenumstr($page,$total,$phpfile,$pagenum,$pagelen);
    $pageinfo=$pagearray['pagecode'];		
	
$tl->set_file('chooseproject');
$tl->n();
$tl->p();
$db->close();
?>